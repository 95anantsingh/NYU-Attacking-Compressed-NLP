...
def main():
    ...
    model_path = "./lstm_"+args.v+"_sst_model/" + EMBEDDING_TYPE + "_" + "model.th"
    vocab_path = "./lstm_"+args.v+"_sst_model/" + EMBEDDING_TYPE + "_" + "vocab"
    ...
    final_acc = utils.get_accuracy(model, targeted_dev_data, vocab, trigger_token_ids)
    f = open('uat_'+str(args.v)+'_'+str(args.t)+'.txt', 'w')
    f.write(','.join([vocab.get_token_from_index(x) for x in trigger_token_ids]) + ':'+ str(final_acc))
